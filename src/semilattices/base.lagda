---
layout: default
title : semilattices.base module
date : 2021-05-25
...
module semilattices.base where
...
-- Example. We start with a join semilattice operation on ℕ (but we will be more general below).
...
proj : {A : Type α} → 𝟛 → (𝟛 → A) → (𝟚 → A)
proj 𝟎  x = λ {𝟎 → x 𝟏 ; 𝟏 → x 𝟐}
proj 𝟏  x = λ {𝟎 → x 𝟎 ; 𝟏 → x 𝟐}
proj 𝟐  x = λ {𝟎 → x 𝟎 ; 𝟏 → x 𝟏}

subtuple : {A : Type α}{I : Arity}(scope : Pred I β) → (I → A) → (Σ[ i ∈ I ] i ∈ scope) → A
subtuple scope tuple (i , p) = tuple i

restriction : {A : Type α}{I : Arity} → Rel I A → (scope : Pred I ℓ₀) → Rel (Σ[ i ∈ I ] i ∈ scope) A
restriction f scope x = {!!}

op-2-3 : {A : Type α} → Op 𝟛 A → (scope : Pred 𝟛 ℓ₀) → Op (Σ[ i ∈ 𝟛 ] i ∈ scope) A
op-2-3 f scope x = f {!!}

IsAssociative : {A : Type α} → Op 𝟚 A → Type α
IsAssociative{A = A} 𝑓 = ∀ (x : 𝟛 → A)
 → (𝑓 λ{𝟎 → (𝑓 (proj 𝟐 x)) ; 𝟏 → x 𝟐}) ≡ (𝑓 λ {𝟎 → x 𝟎 ; 𝟏 → 𝑓 (proj 𝟎 x)})


-- The operational signature (semilattices have one binary operation)

-- Greatest common divisor semilattice
-- d = m ∧ n  ⇔   d is the largest natural number that divides both m and n.
𝑨 : structure {ℓ₀} Sig∅ {ℓ₁} Sig-2
𝑨 = record { univ = ℕ ; srel = λ () ; sop = λ 𝑓 x → gcd (x 𝟎) (x 𝟏) }


record IsSemigroup (𝑨 : structure {α} Sig∅ {ℓ₀} Sig-2) : Type α where
 field
  assoc : (𝑓 : symbol Sig-2) → IsAssociative (sop 𝑨 𝑓)

--   open IsMagma isMagma public


-- -- The semilattice laws.
-- record IsSemilattice (· : semilattice-op) : Type ? where
--   field
...
-- -- There are two 3-element semilattice
-- 𝟚-semilattice : structure {ℓ₀} semilattice-rels  {ℓ₀} semilattice-ops
-- 𝟚-semilattice = record { univ = 𝟚 ; srel = λ() ; sop = λ ∧ x → min (x 𝟎) (x 𝟏) }
--  where
--  min : 𝟚 → 𝟚 → 𝟚
